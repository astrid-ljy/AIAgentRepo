...
def llm_json(system_prompt: str, user_payload: str) -> dict:
    ...
    Shows raw text on parse errors instead of crashing the UI.
    """
    ...
    except Exception as e_jsonmode:
            ...
            if m["artifacts"]:
                with st.expander("Artifacts", expanded=False):
                    for k, v in m["artifacts"].items():
                        ...
                        # Only render as JSON when it's actual JSON (dict/list)
...
st.subheader("Chat")
render_chat()
user_prompt = st.chat_input("Ask a business question (BA focuses on finance; DS proposes modeling)…")
